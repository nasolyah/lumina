...
- Нужна иерархия (раздел → под-разделы): если в разделе 2+ самостоятельных пункта
  (нумерованные пункты 1.1/1.2, подзаголовки, элементы списка, абзацы на разные
  подтемы) — каждый пункт делай под-разделом в "children" с коротким понятным
  названием по смыслу. Только раздел из одного цельного абзаца — без детей.
...
разделы и их ПОДРАЗДЕЛЫ, как это сделал бы внимательный читатель, составляя
оглавление-карту: ветки верхнего уровня и конкретные пункты внутри них.
...
- НАЗВАНИЕ ДОКУМЕНТА и его шапка (город, дата, номер, реквизиты над текстом) — это
  "root", а НЕ отдельный раздел: не делай ветку вроде «Заголовок документа»/«Шапка»,
  а относи эти строки к ПЕРВОМУ разделу;
- порядок сохраняй; 4-8 разделов ВЕРХНЕГО уровня — там не мельчи и не склеивай всё
  в один (это правило только про верхний уровень);
- ВТОРОЙ УРОВЕНЬ ОБЯЗАТЕЛЕН: если в разделе 2+ самостоятельных пункта — нумерованные
  пункты (1.1, 1.2…), подзаголовки, элементы списка, отдельные абзацы на разные
  подтемы, отдельные таблицы — каждый такой пункт становится ПОДРАЗДЕЛОМ в "children"
  с коротким понятным названием (2-5 слов, по смыслу пункта, а не «Пункт 1.1»).
  Строку-заголовок раздела оставляй в block_ids самого раздела, строки пунктов —
  в block_ids подразделов. Раздел из одного цельного абзаца — без "children";
- третий уровень — только если у подраздела действительно есть свои подпункты;
...
{"root":"тема документа (2-6 слов)",
 "children":[
   {"title":"Оценивание","block_ids":["b0"],"children":[
      {"title":"Экзамен","block_ids":["b1","b2"]},
      {"title":"Домашние задания","block_ids":["b3","b4"]}
   ]},
   {"title":"Правила курса","block_ids":["b5"],"children":[
      {"title":"Посещаемость","block_ids":["b6","b7"]},
      {"title":"Сдача после дедлайна","block_ids":["b8"]}
   ]},
   {"title":"Цельный абзац без пунктов","block_ids":["b9","b10"]}
 ]}"""
